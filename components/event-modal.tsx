...
import { X, CalendarIcon, Clock, MapPin, Users, AlignLeft, Globe } from "lucide-react";
...
export function EventModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const eventData: CalendarEvent = {
      id: mode === "edit" && event && event.id ? event.id : Math.floor(Math.random() * 1000),
      title,
      startDate,
      endDate,
      timezone,
      ...
      <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl shadow-xl w-full max-w-md mx-4 overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-white/20">
            ...
            <div className="w-10"></div> {/* Placeholder to keep title centered if needed, or remove if not */}
          </div>
          {/* Barra di navigazione eventi multipli */}
          {(typeof eventIndex === "number" && typeof eventCount === "number" && eventCount > 1) && (
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <label htmlFor="all-day-switch" className="flex items-center cursor-pointer">
              <div className="relative">
                <input type="checkbox" id="all-day-switch" className="sr-only" checked={isAllDay} onChange={() => setIsAllDay(!isAllDay)} />
                <div className={`block w-10 h-6 rounded-full ${isAllDay ? 'bg-pink-500' : 'bg-gray-600'}`}></div>
                <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${isAllDay ? 'translate-x-full' : ''}`}></div>
              </div>
            </label>
          </div>

          <div className="flex items-center gap-3">
            <CalendarIcon className="h-5 w-5 text-white/70 invisible" /> {/* Invisible icon for alignment */}
            <div className="flex w-full gap-2">
              <div className="w-[70%]">
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      {startDate && !isNaN(startDate.getTime()) ? format(startDate, "EEE d MMM yyyy") : "Select start date"}
                    </Button>
                  </PopoverTrigger>
              ...
              {!isAllDay && (
                <div className="w-[30%]">
                  <Select
                    value={startDate && !isNaN(startDate.getTime()) ? format(startDate, "HH:mm") : "08:50"}
                    onValueChange={(value) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex w-full gap-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className="w-full justify-start text-left font-normal text-white hover:bg-white/20 bg-white/10 border border-white/20 px-3 py-2 h-10"
                    >
                      {endDate && !isNaN(endDate.getTime()) ? format(endDate, "EEE d MMM yyyy") : "Select end date"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
              ...
              {!isAllDay && (
                <div className="w-[30%]">
                  <Select
                    value={endDate && !isNaN(endDate.getTime()) ? format(endDate, "HH:mm") : "09:45"}
                    onValueChange={(value) => {
                      const [hours, minutes] = value.split(':').map(Number);
          ...
          <div className="flex items-center gap-3">
            ...
            <Button variant="ghost" className="text-white hover:bg-white/20 bg-white/10 border border-white/20 px-3 py-2 h-auto justify-start w-full">
                Aggiungi persone
            </Button>
          </div>
